...
class ShiaPrayTimes {
	...
	times(date, lat, lng, timeZone, format, dstType, dstDates) {
		...
		var libTimes = this.prayTimes.getTimes(date, [lat, lng], timeZone, 0, format);

		this.date = new Date(date);
		this.longitude = lng;
		this.latitude = lat;
		this.julianDate = this.prayTimes.julian(date.getFullYear(), date.getMonth() + 1, date.getDate());;
		...
		var farsiDayNames = ['یکشنبه', 'دوشنبه', 'سه شنبه', 'چهارشنبه', 'پنج شنبه', 'جمعه', 'شنبه'];
		var frenchDayNames = ["Dimanche", "Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi"];
		var arabicDayNames = ["الأحد", "الإثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت"];
		...
		goodTimes.date = date.getFullYear() + '-' + Number(date.getMonth() + 1) + '-' + date.getDate();
		...
		goodTimes.shadow = Math.abs(1 / Math.tan(parseFloat(goodTimes.sunaltitude) * (Math.PI / 180)));
		...
		goodTimes.shadow = goodTimes.sunaltitude.indexOf('N') > 0 ?
			(Math.abs(1 / Math.tan(parseFloat(goodTimes.sunaltitude) * (Math.PI / 180))) * 100).toFixed(0) + "% S" :
			(Math.abs(1 / Math.tan(parseFloat(goodTimes.sunaltitude) * (Math.PI / 180))) * 100).toFixed(0) + "% N";
	...
	addMinutes(time, mins) {
		...
		return (d.getHours() + ':' + ((outMinutes < 10) ? '0' + outMinutes : outMinutes));
	}
	...
	getDaySequence() {
		...
		if (this.isLeapYear) {
			return Math.floor(275 * month / 9) - Math.floor((month + 9) / 12) + day - 30;
		} else {
			return Math.floor(275 * month / 9) - 2 * Math.floor((month + 9) / 12) + day - 30;
		}
	}
	...
	getSunAngleTime(angle, timeStr, ccw) {
		var newTime = this.prayTimes.sunAngleTime(angle, this.timeStringToDayPortion(timeStr), ccw);
	...
	getAsrTime(factor, timeStr) {
		var newTime = this.prayTimes.asrTime(factor, this.timeStringToDayPortion(timeStr));
		newTime += this.timeZone - this.longitude / 15 + (this.dst ? 1 : 0);
		return this.prayTimes.getFormattedTime(newTime, this.format);
	}

	getSunAltitude(timeStr) {
		var decl = this.prayTimes.sunPosition(this.julianDate + this.timeStringToDayPortion(timeStr)).declination;
	...
	qiblaTime(timeStr) {
		...
		var decl = this.prayTimes.sunPosition(this.julianDate + this.timeStringToDayPortion(timeStr)).declination * Math.PI / 180;
		...
		var result = (Math.abs(meccalong - GL) < 90) ? this.prayTimes.getFormattedTime(X, this.format) : 'X';
		return result;
	}
	...
	getQiblaDirection(latitude, longitude) {
		...
		var XQ = Math.sin(TMA) * Math.cos(GP) - Math.cos(TMA) * Math.sin(GP) * Math.cos(CAB);
